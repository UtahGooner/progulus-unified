...
WHERE (ISNULL(NULLIF(:artist, '')) OR s.artist REGEXP :artist OR s.artist COLLATE utf8_general_ci = :artist)
  AND (ISNULL(NULLIF(:country, '')) OR s.label REGEXP :country)
  AND (ISNULL(NULLIF(:album, '')) OR s.album REGEXP :album OR s.album COLLATE utf8_general_ci = :album)
  AND (ISNULL(NULLIF(:title, '')) OR s.title REGEXP :title OR s.title COLLATE utf8_general_ci = :title)
